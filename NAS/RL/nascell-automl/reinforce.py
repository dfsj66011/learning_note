...
class Reinforce:
    ...
    def create_variables(self):
        ...
        with tf.name_scope("predict_actions"):
            ...
            self.action_scores = tf.identity(self.policy_outputs, name="action_scores")  # [?, 1, 8]

            self.predicted_action = tf.cast(tf.scalar_mul(self.division_rate, self.action_scores), tf.int32,
                                            name="predicted_action")                     # [?, 1, 8]
        ...
        with tf.name_scope("compute_gradients"):
            ...
            with tf.variable_scope("policy_network", reuse=True):
                self.logprobs = self.policy_network(self.states, self.max_layers)    # [?, 1, 8]
                print("self.logprobs", self.logprobs)

            # compute policy loss and regularization loss
            self.cross_entropy_loss = tf.nn.softmax_cross_entropy_with_logits(logits=self.logprobs[:, -1, :],   # [?, 8]
                                                                              labels=self.states)               # [?, 8]
