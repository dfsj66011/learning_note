...
def policy_network(state, max_layers):
    ...
    with tf.name_scope("policy_network"):
        nas_cell = tf.contrib.rnn.NASCell(4 * max_layers)  # output_size:8  state_size: LETTMStateTuple(c=8, h=8)
        # outputs: [batch_size, max_time, cell.output_size]  -----> batch, 8, 8
        outputs, state = tf.nn.dynamic_rnn(nas_cell, tf.expand_dims(state, -1), dtype=tf.float32)    # outputs: [?, 8, 8]  state: c [?, 8]; h [?, 8]
...
def train(mnist):
    ...
    net_manager = NetManager(num_input=784, num_classes=10, learning_rate=0.001,
                             mnist=mnist, bathc_size=100)
